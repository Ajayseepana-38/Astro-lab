...
const stats = [
  { label: "Learning Hours", value: "124h", increase: "+12h this week", icon: Clock, color: "text-blue-400", bg: "bg-blue-400/10" },
  { label: "Current Streak", value: "14 Days", increase: "Personal best: 21", icon: Flame, color: "text-orange-400", bg: "bg-orange-400/10" },
  { label: "Total XP", value: "8,450", increase: "Level 12 Scholar", icon: Trophy, color: "text-amber-400", bg: "bg-amber-400/10" },
  { label: "Courses Completed", value: "4", increase: "Top 15% of learners", icon: Target, color: "text-emerald-400", bg: "bg-emerald-400/10" },
]

const recentActivity = [
  { id: 1, title: "Quantum Mechanics Essentials", type: "course", progress: 68, time: "2 hours ago" },
  { id: 2, title: "Earned 'Fast Learner' Badge", type: "achievement", time: "1 day ago" },
  { id: 3, title: "Completed 'The Standard Model' Quiz", type: "quiz", score: "92%", time: "2 days ago" },
]
          ...
          <section className="rounded-[32px] border border-white/10 bg-slate-900/40 p-6 backdrop-blur-xl sm:p-8">
            ...
            <div className="group relative flex flex-col gap-6 rounded-[24px] border border-white/10 bg-slate-950/60 p-4 transition-all hover:border-cyan-400/30 sm:flex-row sm:items-center">
              <div className="relative h-40 w-full shrink-0 overflow-hidden rounded-[16px] sm:h-32 sm:w-56">
                <img src="https://images.unsplash.com/photo-1635070041078-e363dbe005cb?auto=format&fit=crop&w=600&q=80" alt="Course" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
              ...
              <div className="flex flex-1 flex-col justify-center">
                <div className="mb-2 flex items-center gap-2">
                  <span className="rounded-md bg-white/10 px-2 py-0.5 text-xs text-slate-300">Module 4</span>
                  <span className="text-xs text-cyan-400">Quantum Superposition</span>
                </div>
                <h3 className="text-lg font-bold text-white">Quantum Mechanics Essentials</h3>
              ...
              <div className="shrink-0 sm:ml-4">
                <button className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-white px-6 font-semibold text-slate-950 transition-all hover:bg-slate-200 sm:w-auto">
          ...
          <section className="rounded-[32px] border border-white/10 bg-slate-900/40 p-6 backdrop-blur-xl sm:p-8">
            ...
            <div className="space-y-4">
              {[
                ...
                { rank: 14, name: "Mina C.", xp: "8,450", current: true },
              ].map((user, i) => (
                <div key={user.name} className={cn(
                  "flex items-center justify-between rounded-2xl p-3",
                  user.current ? "bg-cyan-400/10 border border-cyan-400/20" : "bg-white/5 border border-transparent"
                )}>
                  <div className="flex items-center gap-3">
                    <span className={cn(
                    ...
                    <span className={cn("text-sm font-medium", user.current ? "text-cyan-300" : "text-slate-300")}>
                      {user.current ? "You" : user.name}
                    </span>
                  </div>
                  <span className="text-sm font-semibold text-slate-100">{user.xp} XP</span>
                </div>
              ))}
            </div>
